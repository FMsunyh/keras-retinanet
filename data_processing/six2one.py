...
def handle_background(input_path, min_side, max_side,output_path):
    ncount = 10000
    io_utils.mkdir(output_path)
    io_utils.remove_all(output_path)
    for f in os.listdir(input_path):
        ...
        temp=output_path
        output_path = os.path.join(output_path,"background" + "_" + str_date[0:4] + "-" + str_date[4:6] + "-" + str_date[6:8] + "-" + str(ncount) + ".jpg")
...
def rename(src_folder,rename_folder):
    io_utils.mkdir(rename_folder)
    io_utils.remove_all(rename_folder)
    for f in os.listdir(src_folder):
        rename_sub_folder=os.path.join(rename_folder,f)
        io_utils.mkdir(rename_sub_folder)
        src_sub_folder=os.path.join(src_folder,f)
        io_utils.copy_dir(src_sub_folder,rename_sub_folder)
...
def handle_img(img,part):
    scale_rate=0.5+2*random.random()
    #rotate_angle = 360 * random.random()
    rotate_angle = random.sample([0,90,180,270],1)[0]
    transpose_mode = random.sample(['H','V'],1)
    brightness_rate = 0.5+2 * random.random()
    constrast_rate = 0.5+2 * random.random()
    color_rate = 0.5+2 * random.random()
    sharpness_rate =0.5+ 2 * random.random()
    img=fusion_utils.scale(img,scale_rate)    #change the scale_rate
    img=fusion_utils.rotate(img,rotate_angle)
    img=fusion_utils.transpose(img,transpose_mode)
    img=fusion_utils.Enhance_Bright(img,brightness_rate)
    img=fusion_utils.Enhance_Constrast(img,constrast_rate)
    img=fusion_utils.Enhance_Color(img,color_rate)
    img=fusion_utils.Enhance_Sharpness(img,sharpness_rate)
    img= fusion_utils.resize(img, part)
    return img

#paste six images into a background
def paste_images(input_path,bg,six_imgs_dict,output_path):
    global nimgs
    try:
        io_utils.mkdir(output_path)
...
#some paths
parent_path='/home/syh/train_data/fusion/' #change there
...
background_origin_path=os.path.join(background_path,"origin")#should be prepare before
background_handle_path=os.path.join(background_path,"after")
after_paste_path=os.path.join(parent_path,"JPEGImages")
xml_path=os.path.join(parent_path,"Annotations")
str_date = '{year}{month}{day}'.format(year='2018', month='05', day='28')  #改这里，日期


def process(bg_path):
    six_imgs_dict = get_six_imgs(all_labels)
    paste_images(rename_path, bg_path, six_imgs_dict, after_paste_path)

def get_data(bg_data_dir):
    bg_files = os.listdir(bg_data_dir)
    bg_paths = [os.path.join(bg_data_dir, bg) for bg in bg_files]
...
if __name__ == "__main__":
    ...
    # main(background_origin_path)
    single(background_origin_path)
