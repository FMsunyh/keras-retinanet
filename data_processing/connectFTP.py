...
def ftpconnect(host, username, password, port=21):
    ...
    try:
        ...
        ftp_obj.cwd(ftp_obj.pwd()+'/test_ftp')

        # 新建远程目录
        ftp_obj.mkd(ftp_obj.pwd()+'/new_mkdir_file')

        # 删除远程目录
        ftp_obj.rmd(ftp_obj.pwd()+'/new_mkdir_file')
...
if __name__ == '__main__':
    ftp = ftpconnect(host="192.168.1.210", username="syh", password="zjai0301")
    downloadfile(ftp_obj=ftp, remotepath='/home/syh/train_data/test.zip', localpath='/home/syh/train_data/test.zip')
    uploadfile(ftp_obj=ftp, remotepath='/home/syh/train_data/test.zip', localpath='/home/syh/train_data/test.zip')
